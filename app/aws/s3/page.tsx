...
export default function S3Page() {
  ...
  const fetchObjects = async (bucketName: string, prefix: string = '') => {
    try {
      setIsLoading(true);
      const response = await fetch(`http://localhost:3000/api/s3/buckets/${bucketName}?prefix=${prefix}`);
      const data = await response.json();
      if (data.error) throw new Error(data.message);
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      setIsUploading(true);
      const response = await fetch(`http://localhost:3000/api/s3/buckets/${selectedBucket}`, {
        method: 'POST',
        body: formData,
      });
      const data = await response.json();
  ...
  const handleDeleteObject = async (key: string) => {
    ...
    try {
      setIsLoading(true);
      const response = await fetch(`http://localhost:3000/api/s3/buckets/${selectedBucket}?key=${encodeURIComponent(key)}`, {
        method: 'DELETE',
      });
