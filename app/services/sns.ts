...
export async function listTopics(): Promise<SNSResponse> {
  ...
  try {
    const response = await fetch('http://localhost:3000/api/sns/topics');
...
export async function deleteTopic(topicArn: string): Promise<void> {
  ...
  try {
    const response = await fetch(`http://localhost:3000/api/sns/topics/${encodeURIComponent(topicArn)}`, {
      method: 'DELETE',
    });
...
export async function createSubscription(params: CreateSubscriptionParams): Promise<{ subscriptionArn: string }> {
  ...
  try {
    const response = await fetch('http://localhost:3000/api/sns/subscriptions', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
