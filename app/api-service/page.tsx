...
const ApiService = () => {
  ...
  const fetchNamespaceAccounts = async (namespaceId: string) => {
    try {
      const response = await fetch(`http://localhost:5000/namespaces/${namespaceId}/accounts`);
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
  ...
  const fetchNamespaceMethods = async (namespaceId: string) => {
    try {
      const response = await fetch(`http://localhost:5000/namespaces/${namespaceId}/methods`);
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
  ...
  const handleExecute = async (isPaginated: boolean) => {
    ...
    try {
      let endpoint = isPaginated
        ? `http://localhost:5000/execute/paginated`
  ...
  const saveItemsToDynamoDB = async (
    ...
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
      ...
      const savePromises = batch.map(async (item, index) => {
        ...
        console.log('Request URL:', `http://localhost:5000/api/dynamodb/tables/${requestDetails.tableName}/items`);
        // console.log('Request Body:', JSON.stringify(itemData, null, 2));
        console.groupEnd();

        try {
          const dbResponse = await fetch(`http://localhost:5000/api/dynamodb/tables/${requestDetails.tableName}/items`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
